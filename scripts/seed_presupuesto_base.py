...
Agrupa por partida unica (jurisdiccion + entidad + programa + inciso + principal +
parcial + subparcial + fuente + ubicacion) sumando montos.

Uso:
    python scripts/seed_presupuesto_base.py
    python scripts/seed_presupuesto_base.py --anos 2024 2025
    python scripts/seed_presupuesto_base.py --limpiar
"""
...
# CSVs de 2024 y 2025 vienen en millones de pesos — convertir a pesos
ANOS_EN_MILLONES = {2023,2024, 2025}
...
def cargar_csv(path: Path) -> dict[tuple, dict]:
    ...
    logger.info("%s: %d filas -> %d partidas unicas", path.name, filas, len(partidas))
    return partidas


def seed(anos: list[int], limpiar: bool = False) -> None:
    ...
    try:
        ...
        for ano in anos:
            candidatos = [
                Path(f"data/credito-anual-{ano}.csv"),
                Path(f"data/presupuesto_{ano}.csv"),
            ]
            csv_path = next((p for p in candidatos if p.exists()), None)
            if not csv_path:
                logger.warning("No se encontro CSV para ejercicio %s (buscado: %s)", ano, candidatos)
                continue

            existentes = db.query(models.PresupuestoBase).filter_by(ejercicio=ano).count()
            if existentes > 0 and not limpiar:
                logger.info("Ejercicio %s ya tiene %d partidas - saltando (usa --limpiar para reemplazar)", ano, existentes)
                continue

            partidas = cargar_csv(csv_path)
            insertadas = 0

            # Factor de conversion: 2024 y 2025 vienen en millones
            factor = 1000000 if ano in ANOS_EN_MILLONES else 1
            ...
            for clave, datos in partidas.items():
                obj = models.PresupuestoBase(
                    ...
                    monto_original             = datos["credito_presupuestado"] * factor,
                    monto_vigente              = datos["credito_vigente"] * factor,
                )
                db.add(obj)
                insertadas += 1
                ...
                if insertadas % 10000 == 0:
                    db.flush()
                    logger.info("  %s: %d partidas insertadas...", ano, insertadas)

            db.commit()
            logger.info("OK Ejercicio %s: %d partidas insertadas (factor x%d)", ano, insertadas, factor)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Seed presupuesto_base desde CSVs de credito anual")
    p.add_argument("--anos", nargs="+", type=int, default=[2023, 2024, 2025],
                   help="Anos a cargar (default: 2023 2024 2025)")
    p.add_argument("--limpiar", action="store_true",
                   help="Borrar todos los registros existentes antes de insertar")
    args = p.parse_args()

    seed(anos=args.anos, limpiar=args.limpiar)