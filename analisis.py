...
Nota de unidades:
  ...
  - presupuesto_base 2024/2025: millones de pesos → se normalizan × 1.000.000
...
Uso:
  ...
  python analisis.py --query 2 --exportar
"""
...
def q3_original_vs_modificado(exportar: bool = False) -> pd.DataFrame:
    # Normalización: 2024 y 2025 están en millones → × 1.000.000
    # para poder comparar contra modificaciones (siempre en pesos nominales)
    sql = """
        SELECT
            pb.jurisdiccion_id,
            ...
            ROUND(SUM(
                CASE
                    WHEN pb.ejercicio IN (2024, 2025)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--query", type=int, choices=[1, 2, 3],
        help="Correr solo la query indicada (1, 2 o 3). Sin este flag corre las tres."
    )
    p.add_argument(
    ...
    queries = [args.query] if args.query else [1, 2, 3]
